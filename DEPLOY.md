# B2B 产品展示网站 - 部署说明

## 项目已完成的功能

### ✅ 前台网站
1. **Home 页面** - 企业简介和精选产品展示
2. **Products 页面** - 产品列表、分类筛选、搜索功能
3. **Product Detail 页面** - 产品详细信息和询盘功能
4. **About 页面** - 公司介绍、发展历程、资质证书
5. **Contact 页面** - 联系信息和在线询盘表单
6. **响应式设计** - 自适应移动端、平板、PC端

### ✅ 后台管理系统
1. **管理员登录** - 安全认证（默认账号：admin/admin123）
2. **Dashboard** - 统计数据概览
3. **产品管理** - 产品列表、编辑、删除、添加功能
4. **产品图片上传** - 支持上传图片到 Cloudflare R2
5. **询盘管理** - 查看、处理、删除询盘

### ✅ 后端 API
1. **产品 API** - GET/POST/PUT/DELETE 完整 CRUD
2. **询盘 API** - 提交询盘、查询、状态更新
3. **管理员 API** - 登录认证、Token 验证
4. **图片上传 API** - 支持上传图片到 R2 存储

### ✅ 数据库设计
- Products 表 - 产品信息
- Inquiries 表 - 询盘信息
- Admins 表 - 管理员账户

## 部署步骤

### 1. 安装依赖

\`\`\`bash
npm install
\`\`\`

### 2. 创建 D1 数据库

\`\`\`bash
# 创建数据库
wrangler d1 create b2b_database

# 记录输出的 database_id，更新到 wrangler.toml 文件中
\`\`\`

### 3. 更新配置

编辑 `wrangler.toml`，将 `database_id` 替换为步骤2中创建的实际ID：

\`\`\`toml
[[d1_databases]]
binding = "DB"
database_name = "b2b_database"
database_id = "你的-database-id"  # 替换这里
\`\`\`

### 4. 创建 R2 存储桶（用于图片上传）

```bash
# 创建 R2 bucket
wrangler r2 bucket create b2b-product-images
```

确认 `wrangler.toml` 中已配置 R2：

```toml
[[r2_buckets]]
binding = "IMAGES"
bucket_name = "b2b-product-images"
```

### 5. 初始化数据库

\`\`\`bash
# 执行数据库 schema
wrangler d1 execute b2b_database --file=./schema/schema.sql
\`\`\`

### 6. 本地开发测试

\`\`\`bash
npm run dev
\`\`\`

访问 http://localhost:8787 测试网站功能

### 7. 部署到 Cloudflare

\`\`\`bash
npm run deploy
\`\`\`

## 重要说明

### ⚠️ 静态资源处理

由于 Cloudflare Workers 的限制，静态文件（CSS/JS/图片）需要通过以下方式之一处理：

**方案1：使用 Cloudflare KV 存储**
\`\`\`bash
# 创建 KV 命名空间
wrangler kv namespace create "STATIC_ASSETS"

# 上传静态文件
wrangler kv key put --binding=STATIC_ASSETS "/css/main.css" ./public/css/main.css
wrangler kv key put --binding=STATIC_ASSETS "/js/main.js" ./public/js/main.js
wrangler kv key put --binding=STATIC_ASSETS "/js/admin.js" ./public/js/admin.js
\`\`\`

**方案2：使用 Cloudflare R2 存储**
- 上传 public 目录下所有文件到 R2
- 配置自定义域名
- 修改代码中的静态资源引用路径

**方案3（推荐）：内联静态资源**
- CSS 可以直接内联到 HTML 的 `<style>` 标签中
- JavaScript 可以直接内联到 HTML 的 `<script>` 标签中
- 对于小型项目，这是最简单的方案

### 🔐 安全设置

1. **修改管理员密码**
   - 首次登录后，建议在数据库中更新管理员密码
   - 使用 `src/utils/auth.js` 中的 `hashPassword` 函数生成新的密码哈希

2. **JWT Secret**
   - 修改 `src/api/handlers/admin.js` 中的 `JWT_SECRET`
   - 建议使用环境变量存储

3. **环境变量配置**
   在 `wrangler.toml` 中添加：
   \`\`\`toml
   [vars]
   JWT_SECRET = "your-very-secure-secret-key"
   ENVIRONMENT = "production"
   \`\`\`

## 使用说明

### 前台功能

1. **浏览产品**
   - 访问首页查看精选产品
   - 进入 Products 页面浏览所有产品
   - 使用分类和搜索功能筛选产品

2. **产品询盘**
   - 在产品详情页点击 "Send Inquiry"
   - 填写询盘表单提交
   - 或在 Contact 页面提交通用询盘

### 后台管理

1. **登录**
   - 访问 `/admin` 或 `/admin/login`
   - 默认账号：`admin`
   - 默认密码：`admin123`

2. **管理产品**
   - 查看所有产品
   - 编辑产品信息
   - 上传产品图片（支持 JPEG, PNG, GIF, WebP，最大 5MB）
   - 删除不需要的产品
   - 添加新产品

3. **管理询盘**
   - 查看所有询盘
   - 查看询盘详情
   - 更新询盘状态
   - 删除已处理的询盘

## 项目结构

\`\`\`
cf_b2b/
├── src/
│   ├── index.js              # Workers 入口
│   ├── api/
│   │   ├── router.js         # API 路由
│   │   └── handlers/         # API 处理器
│   │       ├── products.js   # 产品 API
│   │       ├── inquiries.js  # 询盘 API
│   │       ├── admin.js      # 管理员 API
│   │       └── upload.js     # 图片上传 API
│   ├── pages/
│   │   ├── router.js         # 页面路由
│   │   ├── layout.js         # 页面布局
│   │   ├── home.js           # 首页
│   │   ├── products.js       # 产品列表页
│   │   ├── product-detail.js # 产品详情页
│   │   ├── about.js          # 关于我们页
│   │   ├── contact.js        # 联系我们页
│   │   ├── admin-login.js    # 管理员登录页
│   │   ├── admin-dashboard.js# 管理后台
│   │   └── static.js         # 静态资源处理
│   └── utils/
│       └── auth.js           # 认证工具
├── public/
│   ├── css/
│   │   └── main.css          # 主样式表
│   └── js/
│       ├── main.js           # 主 JS 文件
│       └── admin.js          # 管理后台 JS
├── schema/
│   ├── schema.sql            # 数据库结构
│   └── init.sql              # 初始化脚本
├── package.json
├── wrangler.toml
└── README.md
\`\`\`

## 下一步优化建议

1. 📧 添加邮件通知功能（询盘提醒）
2. 🔍 增强搜索功能（全文搜索）
3. 📊 添加更多统计图表
4. 🌐 多语言支持
5. 💾 数据导出功能（Excel/CSV）
6. 🔔 实时通知系统
7. 🖼️ 图片编辑功能（裁剪、压缩）
8. 📱 移动端管理后台优化

## 技术支持

如遇问题，请检查：
1. Wrangler CLI 版本是否最新
2. D1 数据库是否正确初始化
3. wrangler.toml 配置是否正确
4. 浏览器控制台是否有错误信息

## License

MIT
